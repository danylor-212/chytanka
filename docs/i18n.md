...
- Keys ending in `_PATTERN` are not printf formats. Date patterns use `{d}` (day), `{dd}` (two-digit day), `{m}` (short month), `{M}` (full month) and `{y}` (year) in any order, so a day-first language may give the month-first and day-first Date Format settings the same pattern.
- Every other translation must keep English's printf conversions (`%d`, `%s`, `%.2f`, `%%`, ...) with the same argument types in the same order. Width, precision and flags may differ (`%5s` for `%s`), but positional forms (`%1$s`) are not supported. `scripts/gen_i18n.py` runs on every build and **fails the build for a bad translation in any language**, not just the ones compiled in, because each one is undefined behaviour at the `snprintf` call. Dropping trailing conversions, or adding conversions to text that English does not format, only warns. The checker's tests run with `python3 -m unittest discover -s test/scripts -p 'test_gen_i18n.py'`.
